...
BulletHandler &BulletHandler::operator=(const BulletHandler &) {
    BulletHandler bulletHandler = BulletHandler();
...
void BulletHandler::updateBullets() {
    for (std::vector<PlayerBullet *>::iterator p_it = m_playerBullets.begin(); p_it != m_playerBullets.end();) {
        if ((*p_it)->getPosition().getX() < 0 || (*p_it)->getPosition().getX() > TheGame::Instance()->getGameWidth() || (*p_it)->getPosition().getY() < 0 || (*p_it)->getPosition().getY() > TheGame::Instance()->getGameHeight() || (*p_it)->dead()) {
            delete *p_it;
    ...
    for (std::vector<EnemyBullet *>::iterator e_it = m_enemyBullets.begin(); e_it != m_enemyBullets.end();) {
        if ((*e_it)->getPosition().getX() < 0 || (*e_it)->getPosition().getX() > TheGame::Instance()->getGameWidth() || (*e_it)->getPosition().getY() < 0 || (*e_it)->getPosition().getY() > TheGame::Instance()->getGameHeight() || (*e_it)->dead()) {
            delete *e_it;
